...
function initialData(type: EditableType): Data {
  ...
  if (type === "payment") return { studentId: "", studentName: "", kind: "Навчання", amount: 290, date: today(), method: "USDT", commissionName: "", commissionPercent: 0, comment: "" };
...
export default function CRMApp({ user }: { user: { name: string; email: string } }) {
  ...
  const stats = useMemo(() => {
    ...
    const commissions = [...groups.payment, ...groups.access].reduce((sum, record) => sum + num(record.data, "amount") * num(record.data, "commissionPercent") / 100, 0);
    ...
    return { revenue, expenses, commissions, profit: revenue - expenses - commissions, expected, balance: Math.max(0, expected - coursePaid), conversion: groups.lead.length ? Math.round(won / groups.lead.length * 100) : 0 };
  }, [groups]);
...
function RecordRow({ record, groups, onEdit, onArchive, onConvert, onDetail }: { record: CrmRecord; groups: Groups; onEdit: (record: CrmRecord) => void; onArchive: (record: CrmRecord) => void; onConvert: (record: CrmRecord) => void; onDetail: (record: CrmRecord) => void }) {
  ...
  return <tr>
    ...
    {record.type === "payment" && <><td>{person(value(d, "studentName"))}</td><td>{value(d, "kind")}</td><td className="money positive">+{money(num(d, "amount"))}<small className="table-sub">Чистими: {money(num(d, "amount") * (1 - num(d, "commissionPercent") / 100))}</small></td><td>{value(d, "date")}</td><td>{value(d, "method")}{num(d, "commissionPercent") > 0 && <small className="table-sub">{value(d, "commissionName") || "Партнер"}: −{num(d, "commissionPercent")}%</small>}</td></>}
...
function RecordModal({ modal, groups, saving, onClose, onSave }: { modal: ModalState; groups: Groups; saving: boolean; onClose: () => void; onSave: (type: EditableType, data: Data, id?: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="modal" onSubmit={submit}><div className="modal-head"><div><span>{record ? "РЕДАГУВАННЯ" : "НОВИЙ ЗАПИС"}</span><h2>{record ? "Редагувати" : "Додати"} {TITLES[type]}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid">
    ...
    {type === "payment" && <>{studentSelect}<Field label="Тип оплати"><Select value={value(data, "kind")} onChange={(e) => set("kind", e.target.value)} options={["Навчання", "Часткова оплата", "Розстрочка", "Доплата", "Інше"]} /></Field><Field label="Сума, $ *"><input required type="number" value={num(data, "amount")} onChange={(e) => set("amount", Number(e.target.value))} /></Field><Field label="Дата"><input type="date" value={value(data, "date")} onChange={(e) => set("date", e.target.value)} /></Field><Field label="Метод"><Select value={value(data, "method")} onChange={(e) => set("method", e.target.value)} options={["USDT", "Карта", "Готівка", "Інше"]} /></Field><Field label="Кому даєш %"><input value={value(data, "commissionName")} onChange={(e) => set("commissionName", e.target.value)} placeholder="Наприклад: блогер" /></Field><Field label="Відсоток партнеру, %"><input type="number" min="0" max="100" value={num(data, "commissionPercent")} onChange={(e) => set("commissionPercent", Number(e.target.value))} /></Field>{num(data, "commissionPercent") > 0 && <div className="field-hint">Партнеру: {money(num(data, "amount") * num(data, "commissionPercent") / 100)} · тобі чистими: {money(num(data, "amount") * (1 - num(data, "commissionPercent") / 100))}</div>}</>}
